...
在开始较复杂的非只读任务前，向用户询问详细的技术细节。完成和用户的alignment后寻求用户同意，并在定义文件夹中写入/修改对应模块的.md定义文件。保持定义简洁详实

开发规则：
1. DRY 原则 (Don't Repeat Yourself)
拒绝知识的重复。系统中的每一个功能点、算法或配置，都应有且仅有一个权威定义。禁止在多个地方手动同步相同的逻辑. `config.yaml` 只保存可调的算法与运行参数，不保存文件系统路径。
